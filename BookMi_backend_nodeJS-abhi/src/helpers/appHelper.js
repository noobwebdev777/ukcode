...
appHelper.responses = {
    unauthorised: appHelper.apiResponse(401, false, 'unauthorised'),
    ok: appHelper.apiResponse(200, true, 'ok'),
}
...
appHelper.tokenValidator = async (token) => {
    ...
    if (!findUser.status) return { status: false, message: 'no user found' }
    try {
