...
businessController.getAllBusinessForAdminPortal = async (req) => {
    try {
        const { skip, limit } = req
        let getAllBusiness = await businessDal.findAllBusinessForAdminPortal(
            skip,
            limit
        )
...
businessController.getBusinessDetails = async (req) => {
    try {
        ...
        let getBusinessDetails = await businessDal.findBusinessDetails(
            businessId
        )
        ...
        for (const portfolioImage of portfolioImageData) {
            ...
            let comments = commentsData.filter(
                (e) => e.imageId && e.imageId.toString() === _id.toString()
            )
            let likes = likesData.filter(
                (e) => e.imageId && e.imageId.toString() === _id.toString()
            )
...
businessController.updateBusiness = async (req) => {
    try {
        ...
        let updateBusiness = await businessDal.findBusinessByIdAndUpdate(
            businessId,
            reqBody
        )
