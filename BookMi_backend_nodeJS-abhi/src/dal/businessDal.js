...
businessDal.findByPhone = async (phone) => {
    try {
        const result = await businessSchema.findOne({ businessMobile: phone })
        if (result) {
            return { status: true, data: result }
        }
        return { status: false, data: result }
    } catch (error) {
        console.log('failed to fetch biz', error)
        return { status: false, data: error.message ? error.message : error }
    }
}

businessDal.findAllBusinessForAdminPortal = async (skip, limit) => {
    try {
        const result = await businessSchema
            .find({ deleted: false })
            .skip(skip)
            .limit(limit)
            .exec()
...
businessDal.findBusiness = async (req) => {
    try {
        ...
        let orArray = []
        if (reqQuery.city) {
            let value = String(reqQuery.city).replace(
                /([.*+?=^!:${}()|[\]\/\\])/g,
                '\\$1'
            )
            query.push({
        ...
        if (reqQuery.categoryId) {
            let business = await serviceDal.getBusinessByCategory(
                reqQuery.categoryId
            )
            let businessIds = []
            ...
            if (businessIds.length > 0) {
                query.push({
                    $or: [
                        { categoryId: ObjectId(reqQuery.categoryId) },
                        { _id: { $in: businessIds } },
                    ],
                })
            } else {
        ...
        if (reqQuery.businessName) {
            let value = String(reqQuery.businessName).replace(
                /([.*+?=^!:${}()|[\]\/\\])/g,
                '\\$1'
            )
            orArray.push({
            ...
            let businessService = await serviceDal.searchService(
                reqQuery.businessName
            )
            let businessIds = []
            businessService.data.map((e) => businessIds.push(e.businessId))
            if (businessIds.length > 0)
                orArray.push({ _id: { $in: businessIds } })
            ...
            .addFields({
                flashSale: {
                    $filter: {
                        input: '$offers',
                        as: 'offers',
                        cond: { $eq: ['$$offers.type', 'Flash_Sale'] },
                    },
                },
            })
            .addFields({
                happyHours: {
                    $filter: {
                        input: '$offers',
                        as: 'offers',
                        cond: { $eq: ['$$offers.type', 'Happy_hours'] },
                    },
                },
            })
            .addFields({
                lastMinuteDiscount: {
                    $filter: {
                        ...
                        as: 'offers',
                        cond: {
                            $eq: ['$$offers.type', 'Last_Minute_Discount'],
                        },
                    },
                },
            })
...
businessDal.findBusinessCopy = async (req, additionalFilter) => {
    try {
        ...
        const newlyAdded = reqQuery.newlyAdded
        let query = [{ deleted: false }]
        if (additionalFilter) {
        ...
        if (reqQuery.location && reqQuery.nearBy === 'true') {
            ...
            result = result.near({
                ...
                distanceField: 'distance',
                near: { type: 'Point', coordinates: loc },
                maxDistance: config.distanceForBusinessDiscover,
                spherical: true,
            })
        }
        const sortBy = newlyAdded ? { createdAt: 1 } : { distance: 1 }
        result = result
            .match({ $and: query })
            .sort(sortBy)
...
businessDal.myBusiness = async (req) => {
    try {
        ...
        let result = await businessSchema
            .aggregate()
            .match({ $and: query })
            .lookup({
                from: 'businessservices',
...
businessDal.findBusinessByIdAndUpdate = async (id, data) => {
    try {
        let result = await businessSchema.findByIdAndUpdate(id, data, {
            new: true,
        })
        if (result) {
            return { status: true, data: result }
