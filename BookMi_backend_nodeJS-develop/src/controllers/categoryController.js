...
categoryController.createCategory = async (req) => {
  try {
    ...
    if(!createCategory.status){
      return appHelper.apiResponse(200, false, "failed to save category", "failed to save category");
    }
...
categoryController.getAllCategory = async (req) => {
  try {
    ...
    if(!getAllCategory.status){
      return appHelper.apiResponse(200, false, "failed to get category", "failed to get category");
    }
...
categoryController.getCategoryDetails = async (req) => {
  try {
    ...
    let getCategoryDetails = await categoryDal.findCategoryDetails(categoryId)
    if(!getCategoryDetails.status){
      return appHelper.apiResponse(200, false, "failed to get category details", "failed to get category details");
    }
    return appHelper.apiResponse(200, true, "success", getCategoryDetails.data);
  } catch (error) {
    console.log(" Failed", error);
    return appHelper.apiResponse(500, false, error.message ? error.message : "internal server error");
  }
...
categoryController.updateCategory = async (req) => {
  try {
    ...
    let updateCategory = await categoryDal.findCategoryByIdAndUpdate(categoryId, reqBody)
...
categoryController.deleteCategory = async (req) => {
  try {
    ...
    if(!deleteCategory.status){
      return appHelper.apiResponse(200, false, "failed to delete category", "failed to delete category");
    }
