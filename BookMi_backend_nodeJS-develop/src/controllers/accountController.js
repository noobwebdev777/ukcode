...
accountController.login = async (req) => {
  try {
    ...
    if (!reqBody.type) {
      return appHelper.apiResponse(200, false, "Login type is required", undefined, 'Login type is required');
    }
    let loginData;
    switch (reqBody.type) {
      case 'normal':
      ...
      case 'apple':
        ...
        break;
      default:
        return appHelper.apiResponse(200, false, "Invalid login type", undefined, 'Invalid login type');
    }
    if (!loginData.status) {
      return appHelper.apiResponse(200, false, loginData.message, loginData.data);
    }
    return appHelper.apiResponse(200, true, loginData.message, loginData.data);
  }
  catch (error) {
...
accountController.register = async (req) => {
  try {
    ...
    return appHelper.apiResponse(200, true, "Register Success", saveUserData.data);
  } catch (error) {
...
accountController.forgotPassword = async (req) => {
  try {
    ...
    let userExistValidation = await accDal.findAccountByEmail(payload.email);
    if (!userExistValidation.status) {
      return appHelper.apiResponse(200, true, "Please Follow your email to reset your password");
    }
    ...
    let resetPass = await accDal.findAccountByIdAndUpdate(accountId, updatePayload);
    if (!resetPass.status) {
...
accountController.forgotPasswordUsingToken = async (req) => {
  try {
    ...
    if (!reqBody.currentPassword || !reqBody.newPassword) {
      return appHelper.apiResponse(200, false, "Current Password and New Password is required to perform the operation ");
    }

    if (reqBody.currentPassword === reqBody.newPassword) {
      return appHelper.apiResponse(200, false, "Current Password and New Password should not be same");
    }
    let userExistValidation = await accDal.findAccountById(accountId);
    if (!userExistValidation.status) {
      return appHelper.apiResponse(200, false, "Invalid User");
    }
    let comparePassword = await bcrypt.compare(reqBody.currentPassword, userExistValidation.data.password);
    if (!comparePassword) {
    ...
    let resetPass = await accDal.findAccountByIdAndUpdate(accountId, { password: hashedPassword });
    if (!resetPass.status) {
      return appHelper.apiResponse(500, false, "something went wrong");
    }
...
accountController.resetPassword = async (req) => {
  try {
    ...
    if (reqBody.password1 != reqBody.password2) {
      return appHelper.apiResponse(200, false, "password and confirm password does not match")
    }
...
accountController.updateMyProfile = async (req) => {
  try {
    ...
    let updateProfile = await accDal.findAccountByIdAndUpdate(accountId, JSON.parse(JSON.stringify(payload)));
...
accountController.deleteAccount = async (req) => {
  try {
    ...
    let deleteUserBookings = await bookingDal.deleteAllBookingForUser(accountId);
    let deleteUserBusiness = await businessDal.deleteAllBusinessForUser(accountId);
    let deleteUserBusinessServices = await businessServiceDal.deleteAllBusinessServicesForUser(accountId);
    let deleteUserComments = await commentDal.deleteAllCommentForUser(accountId);
    let deleteUserDevice = await deviceDal.deleteAllDevicesForUser(accountId);
    ...
    let deleteUserReviews = await reviewDal.deleteAllReviewsForUser(accountId);
...
const normalLogin = async (req) => {
  ...
  if (reqBody.type != findUser.data.type) {
    return { status: false, message: `Account is already exist using ${findUser.data.type} login` };
  }
  if (reqBody.role != findUser.data.role) {
    return { status: false, message: `${findUser.data.role} account cannot be used as ${reqBody.role} account` };
  }
  let comparePassword = await bcrypt.compare(reqBody.password, findUser.data.password);
  if (!comparePassword) {
  ...
  let token = await jwt.sign(jwtPayload, findUser.data.secret, {});
  if (!token) {
...
const socialLogin = async (req) => {
  try {
    ...
    if (user.status && reqBody.type != user.data.type) {
      return { status: false, message: `Account is already exist using ${user.data.type} login` };
    }
    if (user.status  && reqBody.role != user.data.role) {
      return { status: false, message: `${user.data.role} account cannot be used as ${reqBody.role} account` };
    }
    if (!user.status) {
      if(!reqBody.email){
        return { status: false, message: `Email is required to register your account` };
      }
      let payload = {
        email: reqBody.email,
        terms: true,
    ...
    let token = await jwt.sign(jwtPayload, user.data.secret, {});
    if (!token) {
      return { status: false, message: "failed to generate token" };
    }
    let response = {
