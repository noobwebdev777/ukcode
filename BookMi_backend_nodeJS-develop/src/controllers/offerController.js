const offerController = new Object();
...
offerController.createOffer = async (req) => {
  try {
    ...
    let checkOffer = await offerDal.findOfferTypeForBusiness(businessId, offerType)    
    if(checkOffer.status){
      return appHelper.apiResponse(200, false, `${offerType} Offer already found for business`, `${offerType} Offer already found for business`);
    }
    let createOffer = await offerDal.saveOffer(reqBody)
    if(!createOffer.status){
      return appHelper.apiResponse(200, false, "failed to save offer", "failed to save offer");
    }
...
offerController.getAllOffer = async (req) => {
  try {
    ...
    if(!getAllOffer.status){
      return appHelper.apiResponse(200, false, "failed to get offer", "failed to get offer");
    }
...
offerController.getOfferDetails = async (req) => {
  try {
    ...
    if(!getOfferDetails.status){
      return appHelper.apiResponse(200, false, "failed to get offer details", "failed to get offer details");
    }
...
offerController.updateOffer = async (req) => {
  try {
    let offerId = req.params.offerId
    let reqBody = req.body
    let updateOffer = await offerDal.findOfferByIdAndUpdate(offerId, reqBody)
...
offerController.deleteOffer = async (req) => {
  try {
    ...
    if(!deleteOffer.status){
      return appHelper.apiResponse(200, false, "failed to delete offer", "failed to delete offer");
    }
...
offerController.applyOffer = async (req) => {
  try {
    ...
    if(!readBusiness.status){
      return appHelper.apiResponse(200, false, "Invalid Business", "Invalid Business");
    }
    ...
    for (const requestedService of requestedServices) {
      ...
      let serviceDetails = businessServices.find(e=> e._id.toString() === serviceId )
      if(!serviceDetails){
        return appHelper.apiResponse(200, false, "Invalid Service", "Invalid Service");
      }
      let offerDetails = businessOffers.find(e=> e._id.toString() === offerId )
      if(!offerDetails){
        return appHelper.apiResponse(200, false, "Invalid offer", "Invalid offer");
      }
      ...
      if (discountType === 'Last_Minute_Discount') {
        discountPercentage = offerDetails.lastMinuteDiscount.discountPercentage;
      }
      if (discountType === 'Happy_hours') {
        let happyHours = offerDetails.happyHours || [];
        let dayOfWeek = new Date(bookingDate).getDay()
        let happyHourDiscount = happyHours.find(e => e.dayOfWeek === dayOfWeek)
        if (!happyHourDiscount || !happyHourDiscount.active) {
          return appHelper.apiResponse(200, false, "Offer is inactive", "Offer is inactive");
        }
        discountPercentage = happyHourDiscount.discountPercentage;
      }
      let offerPrice = discountPercentage ? (serviceDetails.price - ((serviceDetails.price / 100)* discountPercentage)) : serviceDetails.price;
      ...
      serviceDetails.offerPrice = Number(new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2, }).format(offerPrice));
    ...
    return appHelper.apiResponse(200, true, "Offer applied", { totalAmount: totalAmount, totalAmountAfterOffer: totalAmountAfterOffer, totalDiscountAmount: totalDiscountAmount, services: res });
  } catch (error) {
    console.log(" Failed", error);
    return appHelper.apiResponse(500, false, error.message ? error.message : "internal server error");
  }
