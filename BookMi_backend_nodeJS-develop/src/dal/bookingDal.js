...
bookingDal.getMyBookingForBusiness = async (businessId, userId) => {
  try {
    let result = await bookingSchema.findOne({ businessId: ObjectId(businessId), bookedById: ObjectId(userId) }).sort({ createdAt: 1 }).lean()
    if (result) {
      return { status: true, data: result };
    }
    return { status: false, data: result }
...
bookingDal.cancelBookingByBookingId = async (id) => {
  try {
    let result = await bookingSchema.findByIdAndUpdate(id, { status: 'CANCELLED' }, { new: true })
    if (result) {
      return { status: true, data: result };
    }
    return { status: false, data: result }
...
bookingDal.finishedBookingByBookingId = async (id) => {
  try {
    let result = await bookingSchema.findByIdAndUpdate(id, { status: 'FINISHED' }, { new: true })
    if (result) {
      return { status: true, data: result };
    }
    return { status: false, data: result }
...
bookingDal.getMyBookingData = async (bookedById) => {
  try {
    ...
    let result = await bookingSchema.aggregate()
      .match({ $and: query })
      .lookup({ from: "businesses", localField: "businessId", foreignField: "_id", as: "businessDetails" })
      .unwind({ path: '$services', preserveNullAndEmptyArrays: true })
      .lookup({ from: "businessservices", localField: "services.serviceId", foreignField: "_id", as: "services.serviceDetails" })
...
bookingDal.businessBookingDetailsForTimeRange = async (businessId, fromDate, toDate) => {
  try {
    let result = await bookingSchema.aggregate()
      .match({
        'services.start': {
      ...
      .lookup({ from: "businesses", localField: "businessId", foreignField: "_id", as: "businessDetails" })
      .lookup({ from: "accounts", localField: "bookedById", foreignField: "_id", as: "userDetails" })
      .unwind({ path: '$services', preserveNullAndEmptyArrays: true })
      .lookup({ from: "businessservices", localField: "services.serviceId", foreignField: "_id", as: "services.serviceDetails" })
      .unwind({ path: '$businessDetails', preserveNullAndEmptyArrays: true })
...
bookingDal.getBookingForParticularDate = async (startDateTime, endDateTime, timezone, businessId) => {
  try {
    ...
    for (const booking of result) {
      ...
      for (const service of services) {
        res.push({
          ...
          start: new Date(moment.tz(service.start, 'YYYY-MM-DDTHH:mm:ssZ', timezone).format('YYYY-MM-DDTHH:mm:ssZ')).getTime(),
          end: new Date(moment.tz(service.end, 'YYYY-MM-DDTHH:mm:ssZ', timezone).format('YYYY-MM-DDTHH:mm:ssZ')).getTime()
        })
      }
    }
    return res
