...
categoryDal.findCategory = async (req) => {
  try {
    let result = await categorySchema.aggregate()
      .match({ delete: false })
      .sort({ createdAt: 1 })
      .lookup({from : "services", localField : "_id", foreignField : "category", as : "services"})
      .exec()
    if (result) {
      return { status: true, data: result }
    }
...
categoryDal.findCategoryByIdAndUpdate = async (id, data) => {
  try {
    let result = await categorySchema.findByIdAndUpdate(id, data, { new: true })
    if (result) {
      return { status: true, data: result };
    }
    return { status: false, data: result }
